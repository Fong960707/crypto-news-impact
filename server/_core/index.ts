...
async function startServer() {
  ...
  // ─── Scheduled: Fetch & Analyze News ─────────────────────────────────────
  app.post("/api/scheduled/fetch-news", async (req, res) => {
    try {
      ...
      res.json({ ok: true, fetched, analyzed, timestamp: new Date().toISOString() });
    } catch (e: any) {
      console.error("[Scheduled] fetch-news error:", e);
